...
var replyService = (function(){
  ...
  function add(reply, callback, error){
    ...
    $.ajax({
      type: "post",
      url: "/MovieReview.do",
      data: JSON.stringify(review), // data는 내가 전달할 데이터이다.
...
//    $.getJSON("", function(){}.fail(function(){}) // 구조
    ...
    $.getJSON("/movieview/pages/"+ m_cd + "/"+ page +".json",
      function(data){
  ...
  // 댓글 삭제
  function remove(rcode, callback, error){
    console.log("remove...........");
    $.ajax({
      type: "delete",
      url: "/replies/" + rcode,
  ...
  // 댓글 조회
  function get(rcode, callback, error){
    $.get("/movieview/"+ rcode + ".json", function(result){if(callback){callback(result);}})
